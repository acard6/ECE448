...
def minimax(side, board, flags, depth):
    ...
    if side:
        ...
        for move in generateMoves(side,board,flags):
            ...
            minVal, newMoveList, newMoveTree = minimax(newside, newboard, newflags, depth-1)
        ...
        for move in generateMoves(side,board,flags):
            newside, newboard, newflags = chess.lib.makeMove(side, board, move[0], move[1], flags, move[2])
            maxVal, newMoveList, newMoveTree = minimax(newside, newboard, newflags, depth-1)
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if side:
        ...
        for move in generateMoves(side,board,flags):
            newside, newboard, newflags = chess.lib.makeMove(side, board, move[0], move[1], flags, move[2])
            childVAl, newMoveList, newMoveTree = alphabeta(newside, newboard, newflags, depth-1, alpha, beta)
            ...
            if childVAl < value:
              value = childVAl
              moveList.clear()
              newMoveList.insert(0, move)
              moveList = newMoveList.copy()
              if (childVAl <= alpha):
                  break
              beta = min(beta, childVAl) #value
        ...
        for move in generateMoves(side,board,flags):
            ...
            if childVAl > value:
              ...
              if (childVAl >= beta):
                  break
              alpha = max(alpha, childVAl) #value


    #using both >= and <= gives me the wrong first move
    # but only using < and > leaves out a move from the moveTree
    return value, moveList, moveTree
