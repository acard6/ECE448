...
class q_learner():
    ...
    def report_exploration_counts(self, state):
        ...
        '''
        return list(self.N[state[0], state[1], state[2], state[3], state[4]])
    ...
    def choose_unexplored_action(self, state):
        ...
        '''
        items = []
        counts = self.report_exploration_counts(state=state)
        # print(self.N.shape)
        # print("STATE: ", state)
        for i in range(len(counts)):
            # print(self.N[state[0], state[1], state[2], state[3], state[4], i])
            if counts[i] < self.nfirst:
                items.append(i)

        if len(items) == 0:
            return None
        item = random.choice(items)
        self.N[state[0], state[1], state[2], state[3], state[4], item] += 1
        return item-1
    ...
    def report_q(self, state):
        ...
        return list(self.Q[state[0], state[1], state[2], state[3], state[4]])
    ...
    def load(self, filename):
        ...
        None
        '''
        files = np.load(filename, allow_pickle=True)
        self.Q = files['q']
        self.N = files['n']
    ...
    def exploit(self, state):
        ...
        '''
        actions = self.report_q(state)
        output = max(actions)
        return (actions.index(output)-1, output)
    ...
    def act(self, state):
        ...
        '''
        act = self.choose_unexplored_action(state)
        if act != None:
          return act
